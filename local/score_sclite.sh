...
if $use_worldbet; then
  $utils/transnorm.pl -map-from-col 2 -map-to-col 3 -trans-start-col 7 $data/stm $utils/phonemap/timit2worldmap.txt | \
...
if [ $stage -le 1 ]; then
  ...
  if $use_worldbet; then
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/map_ctm.LMWT.log \
       ...
       $utils/transnorm.pl  -map-from-col 2 -map-to-col 3 -trans-start-col 5 -trans-end-col 5 - \
			$utils/phonemap/timit2worldmap.txt '>' $dir/scoring/LMWT.ctm_39phn || exit 1 ;       
  else
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/map_ctm.LMWT.log \
       mkdir $dir/score_LMWT ';' \
       cat $dir/scoring/LMWT.ctm \| \
