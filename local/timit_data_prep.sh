...
utils=`pwd`/utils
conf=`pwd`/conf
use_worldbet=false # can be overriden by path.sh
...
for x in train dev test; do
  ...
  # Do normalization steps.  Convert TIMIT 60 phones -> 48 phones  
  cat ${x}.trans | $local/timit_norm_trans.pl -i - -m $conf/phones.60-48-39.map -to 48 | sort > ${x}.text || exit 1;
  if $use_worldbet; then
    cp ${x}.text ${x}tmp.text
    # Amit: Convert TIMIT 48 -> Worldbet 48
	perl $utils/transnorm.pl -map-from-col 1 -map-to-col 2 -trans-start-col 2 ${x}tmp.text $utils/phonemap/timit2worldmap.txt | sort > ${x}.text || exit 1;   
	rm ${x}tmp.text;
  fi  
